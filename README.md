...
`.github/workflows/monitor.yml` runs `scripts/monitor.mjs` on a schedule.
...
## Known limitation: checks aren't exactly on schedule

GitHub Actions' `schedule` trigger is best-effort, not guaranteed — it can
be delayed or skipped, especially around busy round-number times. In
practice this monitor may check every hour or two rather than exactly on
the configured interval. There's no fix for this from the workflow file
alone; it's a platform limitation. Manually running the workflow (Actions
tab → Run workflow) always works immediately if you want a check right now.

## Adjusting the check interval

Edit the `cron` schedule in `.github/workflows/monitor.yml` (currently
`*/10 * * * *`, every 10 minutes). GitHub Actions schedules are never
guaranteed to run exactly on time (see above), and won't run more often
than every 5 minutes regardless of what the cron expression says.
